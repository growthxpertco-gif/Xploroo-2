/* ==========================================================================
   XPLOROO · Shimla Quiz — interactive engine
   shimla-quiz.js — Drives shimla-quiz.html: intro → 7-minute timed quiz
   ...
   Reads its question data from the global `shimlaQuizQuestions`
   (js/shimla-quiz-data.js, loaded first). Vanilla JS, no dependencies.
   Loaded with `defer`.

   Same architecture as js/japan-quiz.js (intentionally — this quiz reuses
   the proven engine design), but entirely self-contained: its own
   `data-sq-*` selectors, its own `sq-*` CSS classes, and its own
   localStorage key (`xploroo-shimla-quiz-state`). It never reads or
   writes anything the Japan Quiz uses, so the two sessions can never
   collide or interfere with each other.

   Fairness: correct answers live only in `shimlaQuizQuestions` (in memory /
...
(function () {
  ...
  const page = document.querySelector("[data-sq-page]");
  if (!page) return;
  if (typeof shimlaQuizQuestions === "undefined" || !Array.isArray(shimlaQuizQuestions)) return;

  const QUESTIONS = shimlaQuizQuestions;
  const TOTAL = QUESTIONS.length;
  const QUIZ_DURATION_MS = 7 * 60 * 1000;
  const PASSING_SCORE = 35;
  const STORAGE_KEY = "xploroo-shimla-quiz-state";
  const SECTION_ORDER = [];
  QUESTIONS.forEach((q) => {
  ...
  /* ------------------------------------------------------------------ */
  const panels = {
    intro: page.querySelector('[data-sq-panel="intro"]'),
    quiz: page.querySelector('[data-sq-panel="quiz"]'),
    result: page.querySelector('[data-sq-panel="result"]'),
  };
  const startBtn = page.querySelector("[data-sq-start]");
  const timerWrap = page.querySelector("[data-sq-timer-wrap]");
  const timerEl = page.querySelector("[data-sq-timer]");
  const sectionBadgeEl = page.querySelector("[data-sq-section-badge]");
  const progressFillEl = page.querySelector("[data-sq-progress-fill]");
  const progressCountEl = page.querySelector("[data-sq-progress-count]");
  const progressPercentEl = page.querySelector("[data-sq-progress-percent]");
  const sectionIntroEl = page.querySelector("[data-sq-section-intro]");
  const sectionTagEl = page.querySelector("[data-sq-section-tag]");
  const sectionTitleEl = page.querySelector("[data-sq-section-title]");
  const questionEl = page.querySelector("[data-sq-question]");
  const snowEl = page.querySelector("[data-sq-snow]");
  const qnumEl = page.querySelector("[data-sq-qnum]");
  const qnumStrongEl = qnumEl ? qnumEl.querySelector("strong") : null;
  const sectionNameEl = page.querySelector("[data-sq-section-name]");
  const qtextEl = page.querySelector("[data-sq-qtext]");
  const optionsEl = page.querySelector("[data-sq-options]");

  const resultEl = page.querySelector("[data-sq-result]");
  const resultBadgeEl = page.querySelector("[data-sq-result-badge]");
  const resultHeadlineEl = page.querySelector("[data-sq-result-headline]");
  const resultSubheadEl = page.querySelector("[data-sq-result-subhead]");
  const resultScoreEl = page.querySelector("[data-sq-result-score]");
  const statCorrectEl = page.querySelector("[data-sq-stat-correct]");
  const statIncorrectEl = page.querySelector("[data-sq-stat-incorrect]");
  const statPercentEl = page.querySelector("[data-sq-stat-percentage]");
  const statTimeEl = page.querySelector("[data-sq-stat-time]");
  const tryAgainBtn = page.querySelector("[data-sq-try-again]");
  const breakdownToggle = page.querySelector("[data-sq-breakdown-toggle]");
  const breakdownPanel = page.querySelector("[data-sq-breakdown-panel]");
  const breakdownList = page.querySelector("[data-sq-breakdown-list]");
  const resultSnowEl = page.querySelector("[data-sq-result-snow]");
  ...
  /* Dedicated Shimla key — never touches the Japan Quiz's storage.       */
  /* ------------------------------------------------------------------ */
  function saveState() {
    try {
  ...
  /* ------------------------------------------------------------------ */
  /* Snow sparkle burst — a handful of short-lived elements, never a      */
  /* continuous particle loop. Auto-removed once the fall animation ends. */
  /* ------------------------------------------------------------------ */
  function burstSnow(container, count) {
    if (!container || reduceMotion) return;
    for (let i = 0; i < count; i++) {
      const flake = document.createElement("span");
      flake.className = "sq-snow-flake";
      flake.textContent = "❄";
      flake.style.left = 8 + Math.random() * 84 + "%";
      flake.style.setProperty("--sq-snow-x", (Math.random() * 60 - 30).toFixed(0) + "px");
      flake.style.animationDelay = (Math.random() * 300).toFixed(0) + "ms";
      flake.style.fontSize = (0.75 + Math.random() * 0.55) .toFixed(2) + "rem";
      container.appendChild(flake);
      window.setTimeout(() => flake.remove(), 2200);
    }
  }
  ...
  function buildOptions(question) {
    ...
    question.options.forEach((optionText, i) => {
      ...
      btn.innerHTML =
        '<span class="sq-option__letter">' + letters[i] + "</span>" +
        '<span class="sq-option__text"></span>' +
        '<svg class="sq-option__check" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M20 6 9 17l-5-5"/></svg>';
      btn.querySelector(".sq-option__text").textContent = optionText;
  ...
  function renderQuestion(index, opts) {
    ...
    if (!opts.skipAnimation) {
      questionEl.classList.remove(
        "sq-anim-in-slide", "sq-anim-in-depth", "sq-anim-in-fade",
        "sq-flourish-sweep"
      );
      ...
      questionEl.classList.add("sq-anim-in-" + variant);
      if (index % 5 === 2) questionEl.classList.add("sq-flourish-sweep");
      if (index % 8 === 4) burstSnow(snowEl, 6);
    }
  }
  ...
  function playExitThen(callback) {
    ...
    questionEl.classList.remove(
      "sq-anim-in-slide", "sq-anim-in-depth", "sq-anim-in-fade", "sq-flourish-sweep"
    );
    questionEl.classList.add("sq-anim-out-" + variant);
    window.setTimeout(() => {
      questionEl.classList.remove("sq-anim-out-" + variant);
      callback();
    }, 260);
  }
  ...
  function renderResult(r) {
    ...
    if (r.passed) {
      ...
      resultSubheadEl.textContent = "You passed the Xploroo Shimla Quiz";
    } else {
    ...
    QUESTIONS.forEach((q, i) => {
      ...
      li.className = "sq-breakdown__row" + (isCorrect ? "" : " is-wrong");
      li.innerHTML =
        '<span class="sq-breakdown__icon">' + (isCorrect ? "✓" : "✕") + "</span>" +
        '<div class="sq-breakdown__body">' +
        '<p class="sq-breakdown__q">' + (i + 1) + ". " + escapeHtml(q.question) + "</p>" +
        '<p class="sq-breakdown__ans">Your answer: <strong>' + escapeHtml(userText) + "</strong></p>" +
        (isCorrect ? "" : '<p class="sq-breakdown__ans">Correct answer: <strong>' + escapeHtml(correctText) + "</strong></p>") +
        "</div>";
