/* ==========================================================================
   ...
   recent-winners.js — Premium continuous auto-scroll marquee (right → left)
   for the Recent Winners carousel on index.html. Seamless infinite loop,
   drag/swipe (mouse + touch), hover-to-pause, dot pagination. This is the
   exact same proven architecture as the working Influencers carousel
   (js/influencers.js), retargeted to this section's own `data-rw-*` hooks
   and `.rw-*` classes so it never touches the Influencers carousel
   (data-inf-*) or any other section.

   ARCHITECTURE (read this before touching the CSS):
   `.rw-carousel` is the VIEWPORT — fixed size, `overflow: hidden`, and it
   is NEVER transformed. `.rw-track` is the TRACK — the flex row holding
   ...
   while the actual on-screen carousel area goes blank.
...
(function () {
  ...
  const carousel = document.querySelector("[data-rw-carousel]");
  if (!carousel) return;

  const track = carousel.querySelector("[data-rw-track]");
  const dotsWrap = document.querySelector("[data-rw-dots]");
  if (!track) return;
  ...
  function tryStart() {
    ...
    const found = Array.from(
      track.querySelectorAll(".rw-slide:not([data-rw-clone])")
    );
  ...
  function init() {
    /* ------------------------------------------------------------------ */
    /* 1. Clone the full card set exactly once — visual-only duplicates   */
    /*    appended after the originals, so the DOM sequence is:           */
    /*    [1,2,3,4,5,6, 1c,2c,3c,4c,5c,6c]. No winner data is duplicated   */
    /*    anywhere except this throwaway DOM clone; original cards,       */
    /*    links and content are untouched.                                */
    /* ------------------------------------------------------------------ */
    originalSlides.forEach((slide) => {
      const clone = slide.cloneNode(true);
      clone.setAttribute("data-rw-clone", "");
  ...
  /* card set (every card width + the gaps between/after them), read      */
  ...
  function buildDots() {
    ...
    for (let i = 0; i < slideCount; i++) {
      ...
      dot.setAttribute("aria-label", "Go to winner story " + (i + 1));
  ...
  /*    ONLY thing that ever moves `.rw-track`.                           */
  /* ------------------------------------------------------------------ */
  function frame(time) {
