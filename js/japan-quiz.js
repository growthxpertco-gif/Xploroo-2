/* ==========================================================================
   XPLOROO · Japan Quiz — interactive engine
   japan-quiz.js — Drives japan-quiz.html: intro → 7-minute timed quiz
   ...
   Reads its question data from the global `japanQuizQuestions`
   (js/japan-quiz-data.js, loaded first). Vanilla JS, no dependencies.
   Loaded with `defer`.

   Fairness: correct answers live only in `japanQuizQuestions` (in memory /
...
(function () {
  ...
  const page = document.querySelector("[data-jq-page]");
  if (!page) return;
  if (typeof japanQuizQuestions === "undefined" || !Array.isArray(japanQuizQuestions)) return;

  const QUESTIONS = japanQuizQuestions;
  const TOTAL = QUESTIONS.length;
  const QUIZ_DURATION_MS = 7 * 60 * 1000;
  const PASSING_SCORE = 35;
  const STORAGE_KEY = "xploroo-japan-quiz-state";
  const SECTION_ORDER = [];
  QUESTIONS.forEach((q) => {
  ...
  /* ------------------------------------------------------------------ */
  const panels = {
    intro: page.querySelector('[data-jq-panel="intro"]'),
    quiz: page.querySelector('[data-jq-panel="quiz"]'),
    result: page.querySelector('[data-jq-panel="result"]'),
  };
  const startBtn = page.querySelector("[data-jq-start]");
  const timerWrap = page.querySelector("[data-jq-timer-wrap]");
  const timerEl = page.querySelector("[data-jq-timer]");
  const sectionBadgeEl = page.querySelector("[data-jq-section-badge]");
  const progressFillEl = page.querySelector("[data-jq-progress-fill]");
  const progressCountEl = page.querySelector("[data-jq-progress-count]");
  const progressPercentEl = page.querySelector("[data-jq-progress-percent]");
  const sectionIntroEl = page.querySelector("[data-jq-section-intro]");
  const sectionTagEl = page.querySelector("[data-jq-section-tag]");
  const sectionTitleEl = page.querySelector("[data-jq-section-title]");
  const questionEl = page.querySelector("[data-jq-question]");
  const petalsEl = page.querySelector("[data-jq-petals]");
  const qnumEl = page.querySelector("[data-jq-qnum]");
  const qnumStrongEl = qnumEl ? qnumEl.querySelector("strong") : null;
  const sectionNameEl = page.querySelector("[data-jq-section-name]");
  const qtextEl = page.querySelector("[data-jq-qtext]");
  const optionsEl = page.querySelector("[data-jq-options]");

  const resultEl = page.querySelector("[data-jq-result]");
  const resultBadgeEl = page.querySelector("[data-jq-result-badge]");
  const resultHeadlineEl = page.querySelector("[data-jq-result-headline]");
  const resultSubheadEl = page.querySelector("[data-jq-result-subhead]");
  const resultScoreEl = page.querySelector("[data-jq-result-score]");
  const statCorrectEl = page.querySelector("[data-jq-stat-correct]");
  const statIncorrectEl = page.querySelector("[data-jq-stat-incorrect]");
  const statPercentEl = page.querySelector("[data-jq-stat-percentage]");
  const statTimeEl = page.querySelector("[data-jq-stat-time]");
  const tryAgainBtn = page.querySelector("[data-jq-try-again]");
  const breakdownToggle = page.querySelector("[data-jq-breakdown-toggle]");
  const breakdownPanel = page.querySelector("[data-jq-breakdown-panel]");
  const breakdownList = page.querySelector("[data-jq-breakdown-list]");
  const resultPetalsEl = page.querySelector("[data-jq-result-petals]");
  ...
  /* ------------------------------------------------------------------ */
  /* Sakura petal burst — a handful of short-lived elements, never a      */
  ...
  function burstPetals(container, count) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      petal.className = "jq-petal";
      petal.textContent = "🌸";
      petal.style.left = 8 + Math.random() * 84 + "%";
      petal.style.setProperty("--jq-petal-x", (Math.random() * 60 - 30).toFixed(0) + "px");
      petal.style.animationDelay = (Math.random() * 300).toFixed(0) + "ms";
      petal.style.fontSize = (0.85 + Math.random() * 0.6).toFixed(2) + "rem";
  ...
  function buildOptions(question) {
    ...
    question.options.forEach((optionText, i) => {
      ...
      btn.type = "button";
      btn.className = "jq-option";
      btn.setAttribute("data-option-index", String(i));
      btn.innerHTML =
        '<span class="jq-option__letter">' + letters[i] + "</span>" +
        '<span class="jq-option__text"></span>' +
        '<svg class="jq-option__check" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M20 6 9 17l-5-5"/></svg>';
      btn.querySelector(".jq-option__text").textContent = optionText;
  ...
  function renderQuestion(index, opts) {
    ...
    if (!opts.skipAnimation) {
      questionEl.classList.remove(
        "jq-anim-in-slide", "jq-anim-in-depth", "jq-anim-in-fade",
        "jq-flourish-sweep"
      );
      ...
      questionEl.classList.add("jq-anim-in-" + variant);
      if (index % 5 === 2) questionEl.classList.add("jq-flourish-sweep");
      if (index % 8 === 4) burstPetals(petalsEl, 6);
    }
  }
  ...
  function playExitThen(callback) {
    ...
    questionEl.classList.remove(
      "jq-anim-in-slide", "jq-anim-in-depth", "jq-anim-in-fade", "jq-flourish-sweep"
    );
    questionEl.classList.add("jq-anim-out-" + variant);
    window.setTimeout(() => {
      questionEl.classList.remove("jq-anim-out-" + variant);
      callback();
    }, 260);
  }
  ...
  optionsEl.addEventListener("click", (e) => {
    const btn = e.target.closest(".jq-option");
  ...
  function renderResult(r) {
    ...
    if (r.passed) {
      ...
      resultSubheadEl.textContent = "You passed the Xploroo Japan Quiz";
    } else {
      resultBadgeEl.textContent = "🎌";
    ...
    QUESTIONS.forEach((q, i) => {
      ...
      li.className = "jq-breakdown__row" + (isCorrect ? "" : " is-wrong");
      li.innerHTML =
        '<span class="jq-breakdown__icon">' + (isCorrect ? "✓" : "✕") + "</span>" +
        '<div class="jq-breakdown__body">' +
        '<p class="jq-breakdown__q">' + (i + 1) + ". " + escapeHtml(q.question) + "</p>" +
        '<p class="jq-breakdown__ans">Your answer: <strong>' + escapeHtml(userText) + "</strong></p>" +
        (isCorrect ? "" : '<p class="jq-breakdown__ans">Correct answer: <strong>' + escapeHtml(correctText) + "</strong></p>") +
        "</div>";
